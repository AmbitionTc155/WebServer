...
void HttpData::Reset() {
  ...
  /*
  weak_ptr<>.lock 创建新的
  std::shared_ptr对象，它共享被管理对象的所有权。若无被管理对象，即
  *this为空，则返回亦为空的 shared_ptr 。
   */
  if (timer_.lock()) {
...
void HttpData::HandleRead() {
  ...
  do {
    bool zero = false;
    int read_num = Utility::Readn(fd_, in_buffer_, zero);
    ...
    if (state_ == STATE_RECV_BODY) {
      ...
      if (static_cast<int>(in_buffer_.size()) < content_length) break;
      state_ = STATE_ANALYSIS;
    }
  ...
  if (!error_) {
    ...
    if (!error_ && state_ == STATE_FINISH) {
      ...
      if (in_buffer_.size() > 0) {
        //下面这一行，没看懂什么意思
        if (connection_state_ != H_DISCONNECTING) HandleRead();
      }
...
void HttpData::HandleWrite() {
  if (!error_ && connection_state_ != H_DISCONNECTED) {
    __uint32_t &events_ = channel_->GetEvents();
    if (Utility::Writen(fd_, out_buffer_) < 0) {
      perror("writen");
      events_ = 0;
      error_ = true;
    }
    if (out_buffer_.size() > 0) events_ |= EPOLLOUT;
  }
}
...
AnalysisState HttpData::AnalysisRequest() {
  if (method_ == METHOD_POST) {
    ...
    out_buffer_ += string(src_addr, src_addr + sbuf.st_size);
    ;
    munmap(mmapRet, sbuf.st_size);
    return ANALYSIS_SUCCESS;
  }
...
void HttpData::HandleError(int fd, int err_num, string short_msg) {
  ...
  Utility::Writen(fd, send_buff, strlen(send_buff));
